...
```{r}
telmodel <- glm(y ~ ., data = tel_train, family = "binomial")
summary(telmodel)
```
...
#test_label <- tel_test$y1
#train_label <- tel_train$y1
...
```{r}
library(class)
telprediction <- knn(tel_train, tel_test, cl = tel_train$y1, k = 5)

library(gmodels)
CrossTable(telprediction, tel_test$y1)

library(caret)
confusionMatrix(as.factor(telprediction), as.factor(tel_test$y1))
```
...
```{r}
telprediction <- predict(annmodel, tel_test)
decision <- ifelse(telprediction >= .5, 1, 0)
CrossTable(decision, tel_test$y1)

confusionMatrix(as.factor(decision), as.factor(tel_test$y1))
```
